...
class AppInfoService {
  ...
  Future<AscResource> findEditable(String appId) async {
    ...
    // Fall back to the first — PATCH will tell us if it's not editable.
    _log.warn(
        'No PREPARE_FOR_SUBMISSION appInfo; using ${infos.first.id}',
        scope: 'appinfo');
    return infos.first;
  }

  /// Sets category relationships on the editable appInfo. Any of the
  /// category IDs may be null to leave that slot untouched.
  ...
  /// Upsert a locale-specific app-info record. Name + privacyPolicyUrl are
  /// what we surface today; extend as needed (privacyPolicyText,
  /// privacyChoicesUrl, subtitle).
  Future<AscResource?> upsertLocalization({
    ...
    if (name != null && name.isNotEmpty) attrs['name'] = name;
    ...
    try {
      if (found.id.isNotEmpty) {
        ...
        _log.success('$locale: appInfo localization updated',
            scope: 'appinfo');
      ...
      _log.success('$locale: appInfo localization created', scope: 'appinfo');
      return AscResource.fromJson(json['data'] as Map<String, dynamic>);
    } on AscApiException catch (e) {
